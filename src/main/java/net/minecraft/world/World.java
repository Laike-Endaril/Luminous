...
public abstract class World implements IBlockAccess, net.minecraftforge.common.capabilities.ICapabilityProvider
{
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos centerPos)
    {
        ...
        int lightForCenter = getLightFor(lightType, centerPos);
        int rawLightCenter = getRawLight(centerPos, lightType);
        ...
        else if (rawLightCenter < lightForCenter)
        {
            ...
            while (readIndex < writeIndex)
            {
                ...
                if (lightFor == dataLight)
                {
                    setLightFor(lightType, dataPos, 0);

                    if (dataLight > 0)
                    {
                        xDist = Math.abs(dataX - centerX);
                        yDist = Math.abs(dataY - centerY);
                        zDist = Math.abs(dataZ - centerZ);

                        if (xDist + yDist + zDist < updateRange)
                        {
                            for (EnumFacing enumfacing : EnumFacing.values())
                            {
                                ...
                                opacity = Math.max(1, blockState.getBlock().getLightOpacity(blockState, this, pos));
                                lightFor = getLightFor(lightType, pos);
        ...
        while (readIndex < writeIndex)
        {
            ...
            rawLightDataPos = getRawLight(dataPos, lightType);

            if (rawLightDataPos != lightForDataPos)
            {
                setLightFor(lightType, dataPos, rawLightDataPos);
                ...
                if (rawLightDataPos > lightForDataPos)
                {
                    ...
                    if (xDist + yDist + zDist < updateRange && writeIndex < lightUpdateBlockList.length - 6)
                    {
                        for (EnumFacing enumfacing : EnumFacing.values())
                        {
                            xx = dataX + enumfacing.getFrontOffsetX();
